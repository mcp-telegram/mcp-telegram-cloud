...
describe("lifecycle.startDrain", () => {
  ...
  it("exits early with outcome=drained once in-flight requests hit zero", async () => {
    const clock = fakeSleeper();
    let active = 3;
    const reader = () => active;
    // After the health-delay, simulate one request finishing per poll until zero.
    let polls = 0;
  ...
  it("exits with outcome=timeout when in-flight requests never drop to zero", async () => {
    const clock = fakeSleeper();
    const result = await startDrain({
      healthDelayMs: 100,
      timeoutMs: 500,
  ...
  it("open sessions do not hold the drain — only in-flight work does", async () => {
    // The bug this pins: the drain used to wait for open MCP sessions, which a
    // connected-but-idle client keeps alive indefinitely. Every deploy then
    // burned the whole timeout and killed the in-flight work anyway. Production
    // showed it as four consecutive `drain timeout active=11 / active=5`.
    const clock = fakeSleeper();
    const events: string[] = [];
    const result = await startDrain({
      healthDelayMs: 10,
      ...
      activeSessions: () => 11,
      sleep: clock.sleep,
      onProgress: (e, inFlight, sessions) => events.push(`${e}:${inFlight}/${sessions}`),
    });
    assert.equal(result.outcome, "drained");
    assert.equal(result.remaining, 0);
    // Session count is still reported, so the operator sees who got cut.
    assert.deepEqual(events, ["start:0/11", "health-delay-done:0/11", "drained:0/11"]);
  });
  ...
  it("records DRAIN_OUTCOME=timeout when in-flight requests never reach zero", async () => {
    const clock = fakeSleeper();
    await startDrain({
      healthDelayMs: 10,
      timeoutMs: 50,
