...
This doc captures the SigNoz queries and alert conditions for the **SLA & HTTP Health** section of the `MCP Telegram Cloud` dashboard (UUID: `019cef83-4129-7bcd-b4ee-be5ed5c7a6ed`). It complements [rate-limiter-dashboard.md](./rate-limiter-dashboard.md) which focuses on Telegram-side retries; this doc focuses on the cloud HTTP surface (`/mcp`, `/oauth/*`, public pages).
